...
import {
  ...
  BarChartData,
  PieChartData
} from '../utils/components/UI/Charts'
import {
  getMinAndMaxDate,
  getIncrementalDatesArray
} from '@helpers/dates'
...
const locationChartData = computed((): PieChartData => {
  ...
  participants.value.forEach((participant: ParticipantObject): void => {
...
const timeLineChartData = computed((): BarChartData => {
  ...
  const chartDates: string[] = getIncrementalDatesArray(minDateStr, maxDateStr, 8)
  const chartMap: {
      [date: string]: number
  } = {}
...
const buildChartOptions = (title: string) => ({plugins: {title: {text: title, display: true}}})
</script>
    ...
    <div class="2xl:w-1/2 w-3/4 mx-auto">
      <div class="flex flex-center align-items-center justify-between">
        <BarChart :data="timeLineChartData"
          :options="buildChartOptions('Count of participants registered in the directory over time')"
        />
        <PieChart :data="locationChartData"
          :options="buildChartOptions('Countries of participants registered in the directory')"
          :width="350"
        />
      </div>
    </div>
