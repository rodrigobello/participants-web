...
export const getIncrementalDatesArray = (minDateString: string, maxDateString: string, incrementalCount: number): string[] => {
  const minDate = moment(minDateString)
  const days = moment(maxDateString).startOf('day').diff(minDate.startOf('day'), 'days')
  return [...Array(incrementalCount).keys()].map(i => {
    return moment(minDate).add(days/incrementalCount * (i+1), 'days').toISOString()
  })
}
